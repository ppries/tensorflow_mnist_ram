...
def glimpse_sensor(img, norm_loc):
    loc = ((norm_loc + 1) / 2) * mnist_size  # normLoc coordinates are between -1 and 1
    ...
    for k in range(batch_size):
        ...
        one_img = tf.image.pad_to_bounding_box(one_img, offset, offset, max_radius * 2 + mnist_size,
        ...
        zooms.append(tf.pack(img_zooms))
...
def get_glimpse(loc):
    glimpse_input = glimpse_sensor(inputs_placeholder, loc)
...
with tf.Graph().as_default():
    ...
    if load_path and ckpt and ckpt.model_checkpoint_path:
        ...
        for step in range(start_step + 1, max_iters):
            ...
            if step % 20 == 0:
                ...
                f_glimpse_images = np.reshape(f_glimpse_images_fetched, (
                    glimpses + 1, batch_size, depth, sensorBandwidth, sensorBandwidth))  # steps, THEN batch
